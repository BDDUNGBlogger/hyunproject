 ...
 <main class="main-content">
        ...
        <div class="details">
            ...
            <form id="environmentForm" method="post" action="${root}admin/campaign/update/environment">
                <div id="details-content-environment">
                    <table class="menubar" data-type="환경">
                        ...
                        <tbody>
                            <c:forEach var="campaign" items="${envirCamlist}">
                                <tr data-end-date="${campaign.end_date}" data-current-amount="${campaign.current_amount}" data-goal-amount="${campaign.goal_amount}">
                          ...
                          <span><fmt:formatNumber value="${campaign.current_amount}" type="currency" currencySymbol="₩"/></span>
                                    ...
                                    <td class="editable">
                                        <fmt:formatNumber value="${campaign.goal_amount}" type="currency" currencySymbol="₩"/>
            ...
            <div class="form-flex-box">
                ...
                <button type="submit" form="environmentForm" class="OK" style="display: none;">확인</button>
                <button type="button" class="delete">삭제</button>
            </div>
            <form id="deleteenvironmentForm" method="post" action="${root}admin/campaign/delete/environment" class="hidden">
            </form>
        </div>
    </main>
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            rows.forEach(row => {
                ...
                if (endDate < today) {
                    row.classList.add('expired'); // 종료일이 지난 경우 빨간색 배경 추가
                }

                if (currentAmount >= goalAmount) {
                    row.classList.add('exceeded'); // 모금 금액이 목표 금액을 초과한 경우 빨간색 배경 추가
                }
            });
            ...
            document.querySelector('.delete').addEventListener('click', function (event) {
                ...
                if (selectedRows.length > 0) {
                    ...
                    if (!confirmed) {
                        ...
                        var deleteForm = document.getElementById('deleteenvironmentForm');
                        selectedRows.forEach(function (checkbox) {
